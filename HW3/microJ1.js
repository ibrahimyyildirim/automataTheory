...
function withValue() {
   v = identifier();             //değişkeni v'ye atıyoruz
   if (tok.kind == ASSIGN) {     // atama operatörünü kontrol ediyo ve match fonk. giderek pop ediyo
      match(ASSIGN);
      let e = expression();      // atamanın sağıdaki değeri expression olarak alıp e'ye atıyoruz
      return new Assignment(v, e); // çıktı ekranına value ve expression değeri döndürüyoruz
   }
}
